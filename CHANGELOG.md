...
- The unreachable repair could never actually be raised on a household that heals by reloading. Measured live on 2026-09-09 during a deliberate 21-minute power cut of the aquarium light: link drop 22:24:37, `automation.ble_proxy_autoheal` reloaded the entry at 22:35:00 and 22:40:00, every reload built a fresh watcher whose 15-minute countdown restarted at zero, and no repair appeared - and HA's own last-advertisement hint was empty because the fixture had not been heard since the restart. The moment a link first went down is now recorded per BLE address in process-scoped state that survives reloads, is never overwritten by a rebuilt watcher, and the rebuilt watcher arms its timer for the remainder of the window (or raises immediately if the window is already over). This covers the "not in BLE cache, will wait for advertisement" setup path too, where no device object ever exists. The record is cleared the moment the link is healthy and when the entry is removed.
